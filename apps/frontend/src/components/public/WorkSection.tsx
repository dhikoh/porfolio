...
export default function WorkSection({ projects, settings }: { projects: ProjectItem[]; settings?: Record<string, string> }) {
  ...
  // First project gets the 3D scroll showcase, rest go in grid
  const featuredProject = projects[0];
  const gridProjects = projects.slice(1);

  let featuredTags: string[] = [];
  if (featuredProject) {
    try { featuredTags = JSON.parse(featuredProject.tags); } catch { featuredTags = []; }
  }

  return (
    <section id="work" className="bg-[#002329] relative" ref={ref}>
      <div className="absolute inset-0 dot-pattern pointer-events-none" />

      {/* Featured project with 3D ContainerScroll */}
      {featuredProject && (
        <div className="relative z-10">
            ...
            titleComponent={
              <div className="max-w-[1200px] mx-auto px-6 md:px-12">
                <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }}>
                  <h2 className="text-3xl sm:text-4xl md:text-[44px] font-bold text-white tracking-tight mb-4" style={{ fontFamily: 'Satoshi, sans-serif', lineHeight: '1.2em' }}>
                    {settings?.section_work_title || 'Karya Digital.'}
                  </h2>
                  <p className="text-white/60 text-base sm:text-lg leading-relaxed max-w-[600px] mx-auto mb-6">
          ...
          >
            {/* Featured project content inside 3D card */}
            <div className="relative w-full h-full">
              {featuredProject.imageUrl ? (
                <Image
                  src={toDirectImageUrl(featuredProject.imageUrl)}
                ...
                <div className="w-full h-full flex items-center justify-center bg-[#003038] rounded-2xl">
                  <span className="text-[80px] font-bold text-white/10">{featuredProject.title[0]}</span>
                </div>
              )}
              {/* Overlay with project info */}
              <div className="absolute inset-0 bg-gradient-to-t from-[#002329] via-transparent to-transparent rounded-2xl" />
              <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
                <div className="flex flex-wrap gap-2 mb-3">
                  {featuredTags.map((tag) => (
                    <span key={tag} className="px-2.5 py-1 bg-[#002329]/70 backdrop-blur-md text-white/90 text-xs font-medium rounded-full border border-white/10">{tag}</span>
                  ))}
                </div>
                <h3 className="text-2xl md:text-3xl font-bold text-white mb-2" style={{ fontFamily: 'Satoshi, sans-serif' }}>{featuredProject.title}</h3>
                <p className="text-white/60 text-sm md:text-base max-w-xl mb-4">{featuredProject.description}</p>
                <div className="flex items-center gap-4">
                  <a href={`/project/${featuredProject.slug}`} className="inline-flex items-center gap-2 px-6 py-2.5 bg-[#5cf28e] text-[#002329] rounded-full font-bold text-sm hover:bg-[#50c878] transition-all hover:scale-105">
                    {settings?.work_detail_label || 'Lihat Detail'} <ChevronRight className="w-4 h-4" />
                  </a>
                  {featuredProject.liveUrl && (
                    <a href={featuredProject.liveUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-2.5 bg-white/10 text-white rounded-full text-sm font-medium hover:bg-white/20 transition-all">
      ...
      {gridProjects.length > 0 && (
        <div className="max-w-[1200px] mx-auto px-6 md:px-12 relative z-10 pb-24 md:pb-32">
