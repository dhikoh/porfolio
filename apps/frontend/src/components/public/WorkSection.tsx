...
export default function WorkSection({ projects }: { projects: ProjectItem[] }) {
  ...
  return (
    <section id="work" className="py-24 md:py-32 bg-[#09090b] relative" ref={ref}>
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <motion.div className="flex flex-col md:flex-row md:items-end justify-between mb-16 md:mb-20 gap-6" initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }}>
          <div className="max-w-2xl">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-semibold text-white tracking-tight mb-6">Karya Digital.</h2>
            <p className="text-zinc-400 text-base sm:text-lg leading-relaxed">Produk digital yang dibangun untuk memecahkan masalah nyata — dari logistik, edukasi, hingga manajemen peternakan.</p>
          </div>
        </motion.div>
              ...
              <motion.div key={project.id} initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: idx * 0.1 }} className="group relative rounded-3xl bg-zinc-900/30 border border-white/5 overflow-hidden hover:border-white/10 transition-all duration-300">
                <div className="relative aspect-video bg-zinc-800 overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 flex flex-wrap gap-2">
                    {tags.map((tag) => (<span key={tag} className="px-2.5 py-1 bg-zinc-950/70 backdrop-blur-md text-white/90 text-xs font-medium rounded-full border border-white/10">{tag}</span>))}
                  </div>
                </div>
                <div className="p-6 sm:p-8">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    ...
                    {project.liveUrl && (
                      <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="flex-shrink-0 w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-zinc-400 hover:bg-white hover:text-zinc-950 transition-all duration-300">
                        <ExternalLink className="w-4 h-4" />
                      </a>
                    )}
                  </div>
                  {project.domain && <p className="text-sm text-zinc-500 mb-3 font-mono">{project.domain}</p>}
                  <p className="text-zinc-400 text-sm leading-relaxed mb-4">{project.description}</p>
                  <a href={`/project/${project.slug}`} className="inline-flex items-center gap-2 text-sm text-white font-medium hover:gap-3 transition-all">
                    Lihat Detail <ChevronRight className="w-4 h-4 text-zinc-500" />
                  </a>
                </div>
              </motion.div>
            );
