...
const socialIcons: { key: string; label: string; urlPrefix: string; icon: React.ReactNode }[] = [
...
export default function Footer({ settings, profile }: FooterProps) {
  const year = new Date().getFullYear();
  const activeSocials = socialIcons.filter(s => profile && (profile as Record<string, string>)[s.key]);
        ...
        <div className="flex flex-col items-center gap-6">
              ...
              {activeSocials.map((social) => {
                ...
                return (
                  <a key={social.key} href={url} target="_blank" rel="noopener noreferrer" aria-label={social.label}
                    className="w-9 h-9 rounded-full bg-[#03151a]/5 flex items-center justify-center text-[#03151a]/40 hover:text-[#5cf28e] hover:bg-[#002329] transition-all duration-300"
                  >
                    {social.icon}
                  </a>
                );
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 w-full">
            <p className="text-sm text-[#03151a]/60 text-center md:text-left">
              © {year} {settings?.footer_text || profile?.fullName || 'Portfolio'}. All rights reserved.
            </p>
